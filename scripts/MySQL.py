...
def MySQL():
    ...
    encode_user = codecs.encode(user.encode(), "hex").decode()  # Updated encoding to hex
    user_length = len(user)
    temp = user_length - 4
    length = codecs.encode(chr(0xa3+temp).encode(), "hex").decode()  # Updated encoding to hex
    ...
    def get_payload(query):
        if query.strip() != '':
            query = codecs.encode(query.encode(), "hex").decode()  # Updated encoding to hex
            query_length = '{:06x}'.format(int((len(query) / 2) + 1))
            query_length = codecs.decode(query_length, "hex")[::-1].hex()
